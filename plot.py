...
def plot_edges(importance, sub, min_max=None):
    ...
    plt.colorbar(im, cax=ax.cbar_axes[0], format='%0.03f')
...
def plot_frequency_bands_graphs(data_feat, sub):
    ...
    ch_names_renamed =  ch_names = ['Fp1', 'F3', 'C3', 'P3', 'O1', 'Fp2', 'F4', 'C4', 'P4', 'O2', 'F7', 
                'T3', 'T5', 'F8', 'T4', 'T6', 'Fz', 'Pz', 'Cz']
    ...
    fake_info = mne.create_info(ch_names=ch_names_renamed, sfreq=128., ch_types='eeg')
    ...
    img, _ =mne.viz.plot_topomap(fake_evoked.data[:, 4], fake_evoked.info, show=False, axes=ax[4], 
                                 cmap="bwr", vlim=(vmin, vmax), names=ch_names_renamed) 
    #cbar = plt.colorbar(ax=ax, shrink=0.75, orientation='vertical', mappable=img)
    ax[0].set_title('Delta', fontsize=20)
    ax[1].set_title('Theta', fontsize=20)        
    ax[2].set_title('Alpha', fontsize=20)
    ax[3].set_title('Beta', fontsize=20)
    ax[4].set_title('Gamma', fontsize=20)
    cbar = plt.colorbar(img, cax=ax.cbar_axes[0], format='%0.02f')
